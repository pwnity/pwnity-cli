...
╚═╝       ╚══╝╚══╝  ╚═╝  ╚═══╝ ╚══════╝ ╚═╝  ╚═╝ ╚═════╝  ╚══════╝"""
    ...
    def _write_state_to_file(self):
        """Gathers the current state and writes it to the session JSON file."""
        if not self.session:
            return
        ...
        for job in jobs_list:
            with job.lock:
                # Important: Re-check job status inside the lock to get the most current state
        ...
        state = {
            ...
            "proxy_enabled": proxy_config is not None, "jobs": jobs_data, "sessions": all_sessions_data,
            "monitoring_status": {
        ...
        try:
            ...
            print(f"CRITICAL: Failed to write session state to '{state_file_path}': {e}", file=sys.stderr)

    def _sync_session_state_for_ui(self, signal_command_completion=False):
        """
    ...
    def postcmd(self, stop: bool, statement: cmd2.Statement) -> bool:
        ...
        if self.web_ui_mode:
            ...
            self._sync_session_state_for_ui(signal_command_completion=should_refresh)
